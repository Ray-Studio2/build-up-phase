...
struct MVP_Projecttion {
    ...
    glm::mat4 view;
    glm::mat4 proj;
    int isNormalPresent;  // 기존 isNormalPresent을 uniform에서 사용할 정수 값으로 변경
} MVP;
...
{                                   // Vulkan 의 Uniform Buffer Object 는 Descriptor 의 한 종류이다.
    ...
    // Create Descriptor Set Layout             
    {
        VkDescriptorSetLayoutBinding uboLayoutBinding{
            ...
            .stageFlags = VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT,  // ubo는 Vertex Shader와 Fragment Shader 모두에서 사용되므로, VK_SHADER_STAGE_VERTEX_BIT 뿐만 아니라 VK_SHADER_STAGE_FRAGMENT_BIT도 포함해야 함
        };
...
void createGraphicsPipeline()
{
    ...
    VkShaderModule vsModule = spv2shaderModule("hw01_vs.spv");
    VkShaderModule fsModule = spv2shaderModule("hw01_fs.spv");
...
void updateUniformBuffer(float t = 0.0)
{
    ...
    if (!vk.uniformBuffer)          // uniformBuffer 가 없는 경우
    {
        ...
        //MVP.isNormalPresent = isNormalPresent ? 1 : 0;  // bool을 정수로 변환
...
void	keyboard(GLFWwindow* window, int key, int code, int action, int mods)
{
    ...
    if (action == GLFW_PRESS) {
        ...
        case GLFW_KEY_SPACE:
            ...
            std::cout << "Switched to " << (isNormalPresent ? "Normal Rendering" : "Phong Shading") << std::endl;
            updateUniformBuffer();  // 변경된 상태를 uniform buffer에 업데이트
