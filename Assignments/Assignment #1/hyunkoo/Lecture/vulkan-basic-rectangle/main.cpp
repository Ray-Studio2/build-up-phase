    ...
    VkMemoryPropertyFlags reqMemProps)
{
    ...
    uint memTypeIndex = 0;
    {
        ...
        for (auto& [props, _] : std::span<VkMemoryType>(spec.memoryTypes, spec.memoryTypeCount)) {
            //if (isSuppoted[memTypeIndex] && (props & reqMemProps) == reqMemProps) {
            if (isSuppoted[memTypeIndex] && (props == reqMemProps)) {
                break;
            }
            ++memTypeIndex;
        }

        //for (uint i = 0; i < spec.memoryTypeCount; ++i) {
        //    VkMemoryType& props = spec.memoryTypes[i];  // 각 메모리 타입 가져오기
        //    if (isSuppoted[i] && (props.propertyFlags & reqMemProps) == reqMemProps) {
        //        // 정확히 일치하는 메모리 속성을 찾았을 때
        //        memTypeIndex = i;
        //        break;
        //    }
        //}
        ...
        //for (uint i = 0; i < spec.memoryTypeCount; ++i) {
        //    VkMemoryType& props = spec.memoryTypes[i];  // 각 메모리 타입 가져오기
        //    if (isSuppoted[i] && (props.propertyFlags == reqMemProps)) {
        //        // flags가 정확히 일치하는 경우만 통과
        //        memTypeIndex = i;
        //        break;
        //    }
        //}
