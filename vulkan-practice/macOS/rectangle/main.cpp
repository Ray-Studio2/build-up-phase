    ...
    ~Global()
    {
        ...
        if (ON_DEBUG)
        {
            ((PFN_vkDestroyDebugUtilsMessengerEXT)vkGetInstanceProcAddr(
                this->instance, "vkDestroyDebugUtilsMessengerEXT"))(this->instance, this->debugMessenger, nullptr);
        }
...
struct Geometry
{
    ...
    static std::tuple<float*, size_t> getVertices()
    {
        static float data[] = {
            ...
            0.5,  -0.5, 0, 1, 0, 0.5, 0.5, 0, 0, 1, -0.5, 0.5, 0, 0, 1,
        };
    ...
    {
        static uint16_t data[] = {0, 1, 2, 2, 3, 0};
        return {data, sizeof(data)};
    }
...
void createVkInstance(GLFWwindow* window)
{
    ...
    // VK_KHR_portability_enumeration 확장 추가
...
void createSwapChain()
{
    ...
    VkPresentModeKHR presentMode = VK_PRESENT_MODE_FIFO_KHR;
    {
        ...
        vkGetPhysicalDeviceSurfacePresentModesKHR(vk.physicalDevice, vk.surface, &presentModeCount,
                                                  presentModes.data());
...
void createGraphicsPipeline()
{
    ...
    if (vkCreateGraphicsPipelines(vk.device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &vk.graphicsPipeline) !=
        VK_SUCCESS)
    {
...
std::tuple<VkBuffer, VkDeviceMemory> createBuffer(VkDeviceSize size, VkBufferUsageFlags usage,
                                                  VkMemoryPropertyFlags reqMemProps)
{
    ...
    uint memTypeIndex = 0;
    {
        ...
        std::pair<VkMemoryPropertyFlags, uint> tmp(0, 0);
        for (auto& [props, _] : std::span<VkMemoryType>(spec.memoryTypes, spec.memoryTypeCount))
        {
            ...
            {
                if (tmp.first == 0 || props < tmp.first)
                {
                    tmp.first = props;
                    tmp.second = memTypeIndex;
                }
            }
            ++memTypeIndex;
        }
        memTypeIndex = tmp.second;
    }
...
void createVertexBuffer()
{
    ...
    std::tie(vk.vertexBuffer, vk.vertexBufferMemory) =
...
void createUniformBuffers()
{
    ...
    vk.uniformBuffers.resize(1);
    vk.uniformBuffersMemory.resize(1);
    vk.uniformBuffersMapped.resize(1);

    for (size_t i = 0; i < 1; i++)
    {
        std::tie(vk.uniformBuffers[i], vk.uniformBuffersMemory[i]) =
...
{
    std::vector<VkDescriptorSetLayout> layouts(1, vk.descriptorSetLayout);
...
void render()
{
    ...
    vkResetCommandBuffer(vk.commandBuffer, 0);
    {
        ...
        vkCmdBeginRenderPass(vk.commandBuffer, &renderPassInfo, VK_SUBPASS_CONTENTS_INLINE);
        {
            ...
            vkCmdBindDescriptorSets(vk.commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, vk.pipelineLayout, 0, 1,
                                    &vk.descriptorSets[0], 0, nullptr);
