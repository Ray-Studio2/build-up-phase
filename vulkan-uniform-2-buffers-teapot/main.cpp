...
struct Camera {
    glm::vec3 dir = glm::normalize(glm::vec3(0, 0, 0) - glm::vec3(30.0f, 30.0f, 30.0f));
    glm::vec3 pos = glm::vec3(30.0f, 30.0f, 30.0f);
    glm::vec3 upVec = glm::vec3(0.0f, 0.0f, 1.0f);
    glm::vec3 rightVec = glm::normalize(glm::cross(dir, upVec));
} camera;
...
void updateBeforeDrawing()
{
    ...
    MVP.view = glm::lookAt(camera.pos, objectPos, camera.upVec);
...
void	keyboard(GLFWwindow* window, int key, int code, int action, int mods)
{
	if (action == GLFW_PRESS || action == GLFW_REPEAT) {
        ...
        case GLFW_KEY_W:
            camera.pos += camera.upVec * 0.5f;
            objectPos += camera.upVec * 0.5f;
            break;
        case GLFW_KEY_S:
            camera.pos -= camera.upVec * 0.5f;
            objectPos -= camera.upVec * 0.5f;
        ...
        case GLFW_KEY_D:
            camera.pos += camera.rightVec * 0.5f;
            objectPos += camera.rightVec * 0.5f;
            break;
        case GLFW_KEY_A:
            camera.pos -= camera.rightVec * 0.5f;
            objectPos -= camera.rightVec * 0.5f;
